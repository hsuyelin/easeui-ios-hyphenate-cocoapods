...
typedef NS_ENUM(NSInteger, EMAudioSession) {
    EM_DEFAULT = 0, EM_AUDIOPLAYER, EM_AUDIORECORDER
};
...
+ (NSString *)dataPath {
    NSString *dataPath =
            [NSString stringWithFormat:@"%@/Library/appdata/chatbuffer", NSHomeDirectory()];
...
- (void)asyncPlayingWithPath:(NSString *)aFilePath completion:(void (^)(NSError *error))completon {
    ...
    if (isNeedSetActive) {[self setupAudioSessionCategory:EM_AUDIOPLAYER isActive:YES];}
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *wavFilePath =
            [[aFilePath stringByDeletingPathExtension] stringByAppendingPathExtension:@"wav"];
    if ([EMVoiceConverter isMP3File:aFilePath]) {
        ...
        if (![fileManager fileExistsAtPath:wavFilePath]) {
            ...
            if (!covertRet) {
                if (completon) {
                    completon([NSError
                            errorWithDomain:NSEaseLocalizedString(@"error.initRecorderFail",
...
- (void)stopPlayingWithChangeCategory:(BOOL)isChange {
    [EMAudioPlayerUtil stopCurrentPlaying];
    if (isChange) {[self setupAudioSessionCategory:EM_DEFAULT isActive:NO];}
}
...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:NSEaseLocalizedString(@"error.recordStoping",
                            @"Record voice is not over yet")
    ...
    NSString *recordPath =
            [NSString stringWithFormat:@"%@/%@", [EMCDDeviceManager dataPath], fileName];
    [EMAudioRecorderUtil asyncStartRecordingWithPreparePath:recordPath completion:completion];
}
...
- (void)asyncStopRecordingWithCompletion:(void (^)(NSString *recordPath, NSInteger aDuration,
    ...
    if (![self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:NSEaseLocalizedString(@"error.recordNotBegin",
    ...
    if ([_recorderEndDate timeIntervalSinceDate:_recorderStartDate] <
            [EMCDDeviceManager recordMinDuration]) {
        if (completion) {
            error = [NSError errorWithDomain:NSEaseLocalizedString(@"error.recordTooShort",
        ...
        dispatch_after(
                dispatch_time(DISPATCH_TIME_NOW,
                        (int64_t) ([EMCDDeviceManager recordMinDuration] * NSEC_PER_SEC)),
    ...
    [EMAudioRecorderUtil asyncStopRecordingWithCompletion:^(NSString *recordPath) {
        if (completion) {
            if (recordPath) {
                ...
                completion(
                        amrFilePath,
                        (int) [self->_recorderEndDate timeIntervalSinceDate:self->_recorderStartDate],
                        error);
            }
            [weakSelf setupAudioSessionCategory:EM_DEFAULT isActive:NO];
        }
...
- (NSError *)setupAudioSessionCategory:(EMAudioSession)session isActive:(BOOL)isActive {
    ...
    if (isNeedActive) {
        ...
        if (!success || error) {
            error = [NSError
                    errorWithDomain:NSEaseLocalizedString(@"error.initPlayerFail",
