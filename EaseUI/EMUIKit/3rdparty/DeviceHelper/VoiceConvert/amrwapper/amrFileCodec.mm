...
static size_t ReadPCMFrame(short speech[], FILE *fpwave, int nChannels, int nBitsPerSample) {
    ...
    if (nBitsPerSample == 8 && nChannels == 1) {
        ...
        for (x = 0; x < PCM_FRAME_SIZE; x++) {speech[x] = (short) ((short) pcmFrame_8b1[x] << 7);}
        ...
        for (x = 0; x < PCM_FRAME_SIZE; x++) {speech[x] = (short) pcmFrame_16b1[x + 0];}
...
// bps = 8 --> 8 bits
//       16 --> 16 bits
int EM_EncodeWAVEFileToAMRFile(const char *pchWAVEFilename, const char *pchAMRFileName,
        int nChannels, int nBitsPerSample) {
    FILE *fpwave;
    FILE *fpamr;
...
static int ReadAMRFrameFirst(FILE *fpamr, unsigned char frameBuffer[], int *stdFrameSize,
...
static int ReadAMRFrame(FILE *fpamr, unsigned char frameBuffer[], int stdFrameSize,
        unsigned char stdFrameHeader) {
    size_t bytes = 0;
...
int EM_DecodeAMRFileToWAVEFile(const char *pchAMRFileName, const char *pchWAVEFilename) {
    ...
    //NSUserDomainMask, YES);     NSString *documentPath       = [paths objectAtIndex:0];     NSString
    //*docFilePath        = [documentPath stringByAppendingPathComponent:[NSString
    //stringWithFormat:@"%s", pchWAVEFilename]];     NSLog(@"documentPath=%@", documentPath);
    //
...
int isMP3File(const char *filePath) {
    ...
    if (!strncmp(magic, MP3_MAGIC_NUMBER, strlen(MP3_MAGIC_NUMBER))) {isMp3 = 1;}
    fclose(fpamr);
    return isMp3;
}
...
int isAMRFile(const char *filePath) {
    ...
    int isAmr = 0;
    fread(magic, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
    if (!strncmp(magic, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER))) {isAmr = 1;}
    fclose(fpamr);
    return isAmr;
}
