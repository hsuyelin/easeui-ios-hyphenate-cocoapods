...
- (NSMutableArray *)photos {
    if (_photos == nil) {_photos = [[NSMutableArray alloc] init];}
...
- (UINavigationController *)photoNavigationController {
    if (_photoNavigationController == nil) {
        _photoNavigationController =
                [[UINavigationController alloc] initWithRootViewController:self.photoBrowser];
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    if (index < self.photos.count) {return [self.photos objectAtIndex:index];}
...
- (void)showBrowserWithImages:(NSArray *)imageArray {
    if (imageArray && [imageArray count] > 0) {
        ...
        for (id object in imageArray) {
            ...
            if ([object isKindOfClass:[UIImage class]]) {
                CGFloat imageSize =
                        ((UIImage *) object).size.width * ((UIImage *) object).size.height;
                if (imageSize > IMAGE_MAX_SIZE_5k) {
                    photo =
                            [MWPhoto photoWithImage:[self scaleImage:object
                                                             toScale:(IMAGE_MAX_SIZE_5k) / imageSize]];
                } else {
    ...
    [rootController presentViewController:self.photoNavigationController
                                 animated:YES
                               completion:nil];
}
...
- (BOOL)prepareMessageAudioModel:(EaseMessageModel *)messageModel
    ...
    if (messageModel.bodyType == EMMessageBodyTypeVoice) {
            ...
            if (!messageModel.isMediaPlayed) {
                ...
                if (chatMessage.ext) {
                    ...
                    NSMutableDictionary *dic =
                            [NSMutableDictionary dictionaryWithDictionary:chatMessage.ext];
        ...
        if (updateCompletion) {updateCompletion(prevAudioModel, currentAudioModel);}
    }
...
- (EaseMessageModel *)stopMessageAudioModel {
    ...
    if (self.audioMessageModel.bodyType == EMMessageBodyTypeVoice) {
        if (self.audioMessageModel.isMediaPlaying) {model = self.audioMessageModel;}
