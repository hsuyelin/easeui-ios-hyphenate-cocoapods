...
- (NSMutableAttributedString *)attributtedStringFromText:(NSString *)aInputText {
    ...
    for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
        ...
        UIImage *emojiImage;
        NSString *emojiName = @"";
        if (_dict) {emojiName = [_dict objectForKey:subStr];}

        if (emojiName == nil || emojiName.length == 0) {emojiName = subStr;}

        emojiImage = [UIImage ease_imageNamedFromMyBundle:emojiName];
