...
- (instancetype)initWithConversationChatter:(NSString *)conversationChatter
    ...
    if ([conversationChatter length] == 0) { return nil; }
    ...
    if (self) {
        ...
        _isCustomDownload         = !([EMClient sharedClient].options.isAutoTransferMessageAttachments);
...
- (void)viewDidLoad {
    ...
    ease_adjustsScrollViewInsets_NO(self.tableView, self);
    self.view.backgroundColor =
        [UIColor colorWithRed:248 / 255.0
                        green:248 / 255.0
                         blue:248 / 255.0
                        alpha:1.0];
    ...
    EMChatToolbarType barType = self.conversation.type == EMConversationTypeChat
                                    ? EMChatToolbarTypeChat
                                    : EMChatToolbarTypeGroup;
    self.chatToolbar = [[EaseChatToolbar alloc]
        initWithFrame:CGRectMake(
                          0, self.view.frame.size.height - chatbarHeight - iPhoneX_BOTTOM_HEIGHT,
                          self.view.frame.size.width, chatbarHeight)
                 type:barType];
    ...
    _lpgr = [[UILongPressGestureRecognizer alloc] initWithTarget:self
    ...
    [[EaseBaseMessageCell appearance]
        setSendBubbleBackgroundImage:[[UIImage ease_imageNamedFromMyBundle:@"chat_sender_bg"]
                                         stretchableImageWithLeftCapWidth:5
                                                             topCapHeight:35]];
    [[EaseBaseMessageCell appearance]
        setRecvBubbleBackgroundImage:[[UIImage ease_imageNamedFromMyBundle:@"chat_receiver_bg"]
                                         stretchableImageWithLeftCapWidth:35
                                                             topCapHeight:35]];

    [[EaseBaseMessageCell appearance] setSendMessageVoiceAnimationImages:@[
        [UIImage ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_full"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_000"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_001"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_002"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_003"]
    ]];
    [[EaseBaseMessageCell appearance] setRecvMessageVoiceAnimationImages:@[
        [UIImage ease_imageNamedFromMyBundle:@"chat_receiver_audio_playing_full"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_receiver_audio_playing000"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_receiver_audio_playing001"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_receiver_audio_playing002"],
        [UIImage ease_imageNamedFromMyBundle:@"chat_receiver_audio_playing003"]
    ]];

    [[EaseBaseMessageCell appearance] setAvatarSize:40.f];
    [[EaseBaseMessageCell appearance] setAvatarCornerRadius:20.f];

    [[EaseChatBarMoreView appearance] setMoreViewBackgroundColor:[UIColor colorWithRed:240 / 255.0
                                                                                 green:242 / 255.0
                                                                                  blue:247 / 255.0
                                                                                 alpha:1.0]];
...
- (void)setupEmotion {
    if ([self.dataSource respondsToSelector:@selector(emotionFormessageViewController:)]) {
        ...
        EaseEmotionManager *manager = [[EaseEmotionManager alloc]
            initWithType:EMEmotionDefault
              emotionRow:3
              emotionCol:7
                emotions:emotions
                tagImage:[UIImage ease_imageNamedFromMyBundle:emotion.emotionId]];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.scrollToBottomWhenAppear) { [self _scrollViewToBottom:NO]; }
...
- (void)saveChatroom:(EMChatroom *)chatroom {
    ...
    NSString *key          = [NSString
        stringWithFormat:@"OnceJoinedChatrooms_%@", [[EMClient sharedClient] currentUsername]];
    NSMutableDictionary *chatRooms =
        [NSMutableDictionary dictionaryWithDictionary:[ud objectForKey:key]];
...
- (void)joinChatroom:(NSString *)chatroomId {
    ...
    [self showHudInView:self.view
                   hint:NSEaseLocalizedString(@"chatroom.joining", @"Joining the chatroom")];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (weakSelf) {
                ...
                if (error != nil) {
                    [strongSelf
                        showHint:[NSString stringWithFormat:NSEaseLocalizedString(
                                                                @"chatroom.joinFailed",
                ...
                if (!error || (error.code == EMErrorChatroomAlreadyJoined)) {
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        ...
                        [[EMClient sharedClient].roomManager leaveChatroom:chatroomId
                                                                     error:&leaveError];
                        [[EMClient sharedClient].chatManager deleteConversation:chatroomId
...
- (void)didReceiveUserJoinedChatroom:(EMChatroom *)aChatroom username:(NSString *)aUsername {
    ...
    [self showHint:[NSString stringWithFormat:NSEaseLocalizedString(@"chatroom.join",
                                                                    @"\'%@\'join chatroom\'%@\'"),
                                              aUsername, aChatroom.chatroomId]
           yOffset:-frame.size.height + KHintAdjustY];
}

- (void)didReceiveUserLeavedChatroom:(EMChatroom *)aChatroom username:(NSString *)aUsername {
    CGRect frame = self.chatToolbar.frame;
    [self showHint:[NSString stringWithFormat:NSEaseLocalizedString(@"chatroom.leave.hint",
                                                                    @"\'%@\'leave chatroom\'%@\'"),
                                              aUsername, aChatroom.chatroomId]
           yOffset:-frame.size.height + KHintAdjustY];
}
    ...
    if ([_conversation.conversationId isEqualToString:aChatroom.chatroomId]) {
        ...
        if (aReason == EMChatroomBeKickedReasonOffline) {
            UIAlertController *alertController = [UIAlertController
                alertControllerWithTitle:@"(ಥ_ಥ)"
                                 message:[NSString
                                             stringWithFormat:NSEaseLocalizedString(
                                                                  @"chatroom.removeForOffline",
                                                                  nil),
                                                              aChatroom.chatroomId]
                          preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *okAction = [UIAlertAction
                actionWithTitle:NSEaseLocalizedString(@"chatroom.join", @"Join")
            ...
            [alertController
                addAction:[UIAlertAction
                              actionWithTitle:NSEaseLocalizedString(@"alert.cancel", @"Cancel")
                                        style:UIAlertActionStyleCancel
                                      handler:^(UIAlertAction *_Nonnull action) {
                                          [weakself.navigationController popToViewController:self
                                                                                    animated:NO];
            ...
            CGRect frame = self.chatToolbar.frame;
            [self showHint:[NSString stringWithFormat:NSEaseLocalizedString(
                                                          @"chatroom.remove",
                                                          @"be removed from chatroom\'%@\'"),
                                                      aChatroom.chatroomId]
                   yOffset:-frame.size.height + KHintAdjustY];
...
- (NSMutableArray *)atTargets {
    if (!_atTargets) { _atTargets = [NSMutableArray array]; }
    return _atTargets;
}

- (NSMutableDictionary *)easeExt {
    if (!_easeExt) { _easeExt = [NSMutableDictionary dictionary]; }
    return _easeExt;
}
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar {
    ...
    if (_chatToolbar) { [self.view addSubview:_chatToolbar]; }
    ...
    tableFrame.size.height      = self.view.frame.size.height - _chatToolbar.frame.size.height -
                             navigationBarHeight - iPhoneX_BOTTOM_HEIGHT;
    ...
    if ([chatToolbar isKindOfClass:[EaseChatToolbar class]]) {
        ...
        self.chatBarMoreView =
            (EaseChatBarMoreView *)[(EaseChatToolbar *)self.chatToolbar moreView];
...
- (void)_canRecordCompletion:(void (^)(EMRecordResponse))aCompletion {
    ...
    if (videoAuthStatus == AVAuthorizationStatusNotDetermined) {
        ...
        if (aCompletion) { aCompletion(EMRequestRecord); }
...
- (void)showMenuViewController:(UIView *)showInView
    ...
    if (_menuController == nil) { _menuController = [UIMenuController sharedMenuController]; }

    if (_deleteMenuItem == nil) {
        _deleteMenuItem =
            [[UIMenuItem alloc] initWithTitle:NSEaseLocalizedString(@"delete", @"Delete")
                                       action:@selector(deleteMenuAction:)];
    }

    if (_copyMenuItem == nil) {
        _copyMenuItem = [[UIMenuItem alloc] initWithTitle:NSEaseLocalizedString(@"copy", @"Copy")
...
- (NSURL *)_convert2Mp4:(NSURL *)movUrl {
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetHighestQuality]) {
        ...
        NSString *mp4Path = [NSString stringWithFormat:@"%@/%d%d.mp4", [EMCDDeviceManager dataPath],
...
- (void)_customDownloadMessageFile:(EMMessage *)aMessage {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertView *alertView = [[UIAlertView alloc]
                initWithTitle:nil
                      message:NSEaseLocalizedString(
                                  @"message.autoTransfer",
                                  @"Please customize the  transfer attachment method")
                     delegate:nil
            cancelButtonTitle:NSEaseLocalizedString(@"sure", @"OK")
...
- (void)_downloadMessageAttachments:(EMMessage *)message {
    __weak typeof(self) weakSelf = self;
    void (^completion)(EMMessage *aMessage, EMError *error) =
        ^(EMMessage *aMessage, EMError *error) {
            if (!error) {
                [weakSelf _reloadTableViewDataWithMessage:message];
            } else {
                [weakSelf showHint:NSEaseLocalizedString(@"message.thumImageFail",
                                                         @"thumbnail for failure!")];
            }
        };
    ...
    if ([messageBody type] == EMMessageBodyTypeImage) {
        ...
        if (imageBody.thumbnailDownloadStatus > EMDownloadStatusSuccessed) {
            ...
            if (_isCustomDownload) {
                //                [self _customDownloadMessageFile:message];
                [self ease_downloadMessageThumbnailByCustom:message completion:completion];
            } else {
                if (isAutoDownloadThumbnail) {
                    [[[EMClient sharedClient] chatManager] downloadMessageThumbnail:message
                                                                           progress:nil
        ...
        if (voiceBody.downloadStatus > EMDownloadStatusSuccessed) {
            ...
            if (_isCustomDownload) {
                //                [self _customDownloadMessageFile:message];
                [self ease_downloadMessageAttachmentsByCustom:message completion:completion];
            } else {
                if (isAutoDownloadThumbnail) {
                        ...
                        downloadMessageAttachment:message
                                       ...
                                       completion:^(EMMessage *message, EMError *error) {
                                           if (!error) {
                                               ...
                                               [weakSelf showHint:NSEaseLocalizedString(
                                                                      @"message.voiceFail",
                                                                      @"voice for failure!")];
                                           }
                                       }];
...
- (BOOL)shouldSendHasReadAckForMessage:(EMMessage *)message read:(BOOL)read {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.easeHasReadFlag = NO;
        if (message.chatType != EMChatTypeChat || message.isReadAcked ||
        ...
        if (((body.type == EMMessageBodyTypeVideo) || (body.type == EMMessageBodyTypeVoice) ||
             (body.type == EMMessageBodyTypeImage)) &&
            !read) {
            self.easeHasReadFlag = NO;
        } else {
            self.easeHasReadFlag = YES;
        }
    });
    return self.easeHasReadFlag;
}
    ...
    for (NSInteger i = 0; i < [messages count]; i++) {
        ...
        if (_dataSource &&
            [_dataSource respondsToSelector:@selector
                         (messageViewController:shouldSendHasReadAckForMessage:read:)]) {
            isSend = [_dataSource messageViewController:self
                         shouldSendHasReadAckForMessage:message
                                                   read:isRead];
        } else {
            isSend = [self shouldSendHasReadAckForMessage:message read:isRead];
        }

        if (isSend) { [unreadMessages addObject:message]; }
    }
...
- (BOOL)_shouldMarkMessageAsRead {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector
                                    (messageViewControllerShouldMarkMessagesAsRead:)]) {
...
- (void)_locationMessageCellSelected:(id<IMessageModel>)model {
    ...
    EaseLocationViewController *locationController = [[EaseLocationViewController alloc]
        initWithLocation:CLLocationCoordinate2DMake(model.latitude, model.longitude)];
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model {
    ...
    NSString *localPath =
        [model.fileLocalPath length] > 0 ? model.fileLocalPath : videoBody.localPath;
    ...
    if (videoBody.thumbnailDownloadStatus == EMDownloadStatusFailed ||
        ![[NSFileManager defaultManager] fileExistsAtPath:videoBody.thumbnailLocalPath]) {
        [self showHint:@"begin downloading thumbnail image, click later"];
        if (_isCustomDownload) {
            //            [self _customDownloadMessageFile:model.message];
            [self ease_downloadMessageThumbnailByCustom:model.message completion:completion];
        } else {
            [[EMClient sharedClient].chatManager downloadMessageThumbnail:model.message
                                                                 progress:nil
                                                               completion:completion];
        }
        return;
    ...
    [self
        showHudInView:self.view
                 hint:NSEaseLocalizedString(@"message.downloadingVideo", @"downloading video...")];
    if (_isCustomDownload) {
        //        [self _customDownloadMessageFile:model.message];
        [self ease_downloadMessageAttachmentsByCustom:model.message
                                           completion:^(EMMessage *message, EMError *error) {
                                               [weakSelf hideHud];
                                               if (!error) {
                                                   block();
                                               } else {
                                                   [weakSelf showHint:NSEaseLocalizedString(@"message.videoFail",
                                                                                            @"video for failure!")];
                                               }
                                           }];
    } else {
        [[EMClient sharedClient].chatManager
            downloadMessageAttachment:model.message
                             progress:nil
                           completion:^(EMMessage *message, EMError *error) {
                               [weakSelf hideHud];
                               if (!error) {
    ...
    if ([imageBody type] == EMMessageBodyTypeImage) {
        if (imageBody.thumbnailDownloadStatus == EMDownloadStatusSuccessed) {
            if (imageBody.downloadStatus == EMDownloadStatusSuccessed) {
                ...
                NSString *localPath =
                    model.message == nil ? model.fileLocalPath : [imageBody localPath];
            ...
            [weakSelf showHudInView:weakSelf.view
                               hint:NSEaseLocalizedString(@"message.downloadingImage",
                                                          @"downloading a image...")];

            void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage,
                                                                        EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    // send the acknowledgement
                    [weakSelf _sendHasReadResponseForMessages:@[ model.message ] isRead:YES];
                    NSString *localPath = aMessage == nil
                                              ? model.fileLocalPath
                                              : [(EMImageMessageBody *)aMessage.body localPath];
                    if (localPath && localPath.length > 0) {
                        UIImage *image = [UIImage imageWithContentsOfFile:localPath];
                        //                        weakSelf.isScrollToBottom = NO;
                        if (image) {
                            [[EaseMessageReadManager defaultManager]
                                showBrowserWithImages:@[ image ]];
                        } else {
                ...
                [weakSelf
                    showHint:NSEaseLocalizedString(@"message.imageFail", @"image for failure!")];
            };

            if (_isCustomDownload) {
                //                [self _customDownloadMessageFile:model.message];
                [self ease_downloadMessageAttachmentsByCustom:model.message completion:completion];
            } else {
                [[EMClient sharedClient].chatManager downloadMessageAttachment:model.message
                                                                      progress:nil
                                                                    completion:completion];
            }
        } else {
            ...
            if (_isCustomDownload) {
                [self ease_downloadMessageThumbnailByCustom:model.message
                                                 completion:^(EMMessage *message, EMError *error) {
                                                     if (!error) {
                ...
                [[EMClient sharedClient].chatManager
                    downloadMessageThumbnail:model.message
                                    progress:nil
                                  completion:^(EMMessage *message, EMError *error) {
                                      if (!error) {
                                          [weakSelf _reloadTableViewDataWithMessage:model.message];
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model {
    ...
    if (downloadStatus == EMDownloadStatusDownloading) {
        [self showHint:NSEaseLocalizedString(@"message.downloadingAudio",
                                             @"downloading voice, click later")];
        return;
    } else if (downloadStatus == EMDownloadStatusFailed ||
               downloadStatus == EMDownloadStatusPending) {
        [self showHint:NSEaseLocalizedString(@"message.downloadingAudio",
                                             @"downloading voice, click later")];
        //        BOOL isCustomDownload = !([EMClient sharedClient].options.isAutoTransferMessageAttachments);
        if (_isCustomDownload) {
            //            [self _customDownloadMessageFile:model.message];
            [self ease_downloadMessageAttachmentsByCustom:model.message completion:nil];
        } else {
            [[EMClient sharedClient].chatManager downloadMessageAttachment:model.message
                                                                  progress:nil
                                                                completion:nil];
    ...
    if (model.bodyType == EMMessageBodyTypeVoice) {
        ...
        [self _sendHasReadResponseForMessages:@[ model.message ] isRead:YES];
        __weak EaseMessageViewController *weakSelf = self;
        BOOL isPrepare                             = [[EaseMessageReadManager defaultManager]
            prepareMessageAudioModel:model
                updateViewCompletion:^(EaseMessageModel *prevAudioModel,
                                       EaseMessageModel *currentAudioModel) {
                    if (prevAudioModel || currentAudioModel) { [weakSelf.tableView reloadData]; }
                }];
        ...
        if (isPrepare) {
            ...
            [[EMCDDeviceManager sharedInstance]
                asyncPlayingWithPath:model.fileLocalPath
...
- (void)_callMessageCellSelected:(id<IMessageModel>)model {
    if (self.delegate && [self.delegate respondsToSelector:@selector
                                        (messageViewController:didSelectCallMessageModel:)]) {
...
- (void)_loadMessagesBefore:(NSString *)messageId count:(NSInteger)count append:(BOOL)isAppend {
    ...
    void (^refresh)(NSArray *messages) = ^(NSArray *messages) {
        if ([messages count] == 0) {
            !self.ease_pullHistoryMessageFromDBCompleted
                ?: self.ease_pullHistoryMessageFromDBCompleted();
            return;
        }
        dispatch_async(_messageQueue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    if (isAppend) {
                        [strongSelf.messsagesSource
                            insertObjects:messages
                        ...
                        if ([object isKindOfClass:[NSString class]]) {
                            ...
                            [formattedMessages
                                enumerateObjectsWithOptions:NSEnumerationReverse
                                                 usingBlock:^(id model, NSUInteger idx,
                                                              BOOL *stop) {
                                                     if ([model isKindOfClass:[NSString class]] &&
                                                         [timestamp isEqualToString:model]) {
                        ...
                        [strongSelf.dataArray
                            insertObjects:formattedMessages
                                atIndexes:[NSIndexSet
                                              indexSetWithIndexesInRange:NSMakeRange(
                                                                             0, [formattedMessages
                                                                                    count])]];
                    ...
                    [strongSelf.tableView
                        scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.dataArray count] -
                                                                            scrollToIndex - 1
                                                                  inSection:0]
            ...
            for (EMMessage *message in messages) { [weakSelf _downloadMessageAttachments:message]; }

            // send the read acknoledgement
            [weakSelf _sendHasReadResponseForMessages:messages isRead:NO];

            // send pull history messages block
            !self.ease_pullHistoryMessageFromDBCompleted
                ?: self.ease_pullHistoryMessageFromDBCompleted();
        });
    };
            ...
            if (strongSelf) {
                ...
                if (isAppend) {
                    ...
                    if ([object isKindOfClass:[NSString class]]) {
                        ...
                        [formattedMessages
                            enumerateObjectsWithOptions:NSEnumerationReverse
                                             usingBlock:^(id model, NSUInteger idx, BOOL *stop) {
                                                 if ([model isKindOfClass:[NSString class]] &&
                                                     [timestamp isEqualToString:model]) {
...
- (void)keyBoardHidden:(UITapGestureRecognizer *)tapRecognizer {
    if (tapRecognizer.state == UIGestureRecognizerStateEnded) { [self.chatToolbar endEditing:YES]; }
}
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)recognizer {
    if (recognizer.state == UIGestureRecognizerStateBegan && [self.dataArray count] > 0) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector
                                        (messageViewController:canLongPressRowAtIndexPath:)]) {
            canLongPress =
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector
                                        (messageViewController:didLongPressRowAtIndexPath:)]) {
            [_dataSource messageViewController:self didLongPressRowAtIndexPath:indexPath];
        } else {
            id object = [self.dataArray objectAtIndex:indexPath.row];
            if (![object isKindOfClass:[NSString class]]) {
                EaseMessageCell *cell =
                    (EaseMessageCell *)[self.tableView cellForRowAtIndexPath:indexPath];
                [cell becomeFirstResponder];
                _menuIndexPath = indexPath;
                [self showMenuViewController:cell.bubbleView
                                andIndexPath:indexPath
                                 messageType:cell.model.bodyType];
            }
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        EaseMessageTimeCell *timeCell =
            (EaseMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
    ...
    if (_delegate &&
        [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:)]) {
        UITableViewCell *cell =
            [_delegate messageViewController:tableView
                         cellForMessageModel:model];
        if (cell) {
            if ([cell isKindOfClass:[EaseMessageCell class]]) {
                EaseMessageCell *emcell = (EaseMessageCell *)cell;
                if (emcell.delegate == nil) { emcell.delegate = self; }
            }
            return cell;
        }
    }
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector
                                    (isEmotionMessageFormessageViewController:messageModel:)]) {
        ...
        if (flag) {
            ...
            // send cell
            EaseCustomMessageCell *sendCell = (EaseCustomMessageCell *)[tableView
                dequeueReusableCellWithIdentifier:CellIdentifier];

            // Configure the cell...
            if (sendCell == nil) {
                sendCell = [[EaseCustomMessageCell alloc] initWithStyle:UITableViewCellStyleDefault
            ...
            if (_dataSource && [_dataSource respondsToSelector:@selector
                                            (emotionURLFormessageViewController:messageModel:)]) {
                EaseEmotion *emotion =
                    [_dataSource emotionURLFormessageViewController:self
                                                       messageModel:model];
                if (emotion) {
                    UIImage *emotionImage =
                        [UIImage ease_imageNamedFromMyBundle:emotion.emotionOriginal];
                    if (emotionImage) {
                        model.image =
                            [UIImage sd_animatedGIFWithData:UIImagePNGRepresentation(emotionImage)];
                    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector
                                    (messageViewController:heightForMessageModel:withCellWidth:)]) {
            CGFloat height = [_delegate messageViewController:self
                                        heightForMessageModel:model
                                                withCellWidth:tableView.frame.size.width];
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector
                                        (isEmotionMessageFormessageViewController:messageModel:)]) {
            BOOL flag =
                [_dataSource isEmotionMessageFormessageViewController:self
                                                         messageModel:model];
            if (flag) { return [EaseCustomMessageCell cellHeightWithModel:model]; }
        }
...
- (void)imagePickerController:(UIImagePickerController *)picker
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        if ([fileman fileExistsAtPath:videoURL.path]) {
            ...
            if (error) { NSLog(@"failed to remove file, error:%@.", error); }
        }
        ...
        if (url == nil) {
            ...
            if ([[UIDevice currentDevice].systemVersion doubleValue] >= 9.0f) {
                            ...
                            requestImageDataForAsset:asset
                                       ...
                                       resultHandler:^(NSData *data, NSString *uti,
                                           ...
                                           if (data != nil) {
                                               ...
                                               [self showHint:NSEaseLocalizedString(
                                                                  @"message.smallerImage",
                ...
                [alasset assetForURL:url
                         resultBlock:^(ALAsset *asset) {
                             if (asset) {
                                 ALAssetRepresentation *assetRepresentation =
                                     [asset defaultRepresentation];
                                 Byte *buffer          = (Byte *)malloc((size_t)[assetRepresentation size]);
                                 NSUInteger bufferSize = [assetRepresentation
                                       getBytes:buffer
                                     fromOffset:0.0
                                         length:(NSUInteger)[assetRepresentation size]
                                          error:nil];
...
- (void)messageCellSelected:(id<IMessageModel>)model {
    if (_delegate &&
        [_delegate respondsToSelector:@selector(messageViewController:didSelectMessageModel:)]) {
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            if (model.message.direction == EMMessageDirectionReceive &&
                ...
                if ([conferenceId length] > 0) { [self _callMessageCellSelected:model]; }
            }
        } break;
...
- (void)statusButtonSelcted:(id<IMessageModel>)model
            withMessageCell:(EaseMessageCell *)messageCell {
    ...
    __weak typeof(self) weakself = self;
    [[[EMClient sharedClient] chatManager] resendMessage:model.message
                                                progress:nil
                                              completion:^(EMMessage *message, EMError *error) {
                                                  if (!error) {
...
- (void)avatarViewSelcted:(id<IMessageModel>)model {
    if (_delegate && [_delegate respondsToSelector:@selector
                                (messageViewController:didSelectAvatarMessageModel:)]) {
...
- (void)chatToolbarDidChangeFrameToHeight:(CGFloat)toHeight {
    [UIView animateWithDuration:0.3
                     animations:^{
                         ...
                         rect.origin.y    = [self ease_navigationBarHeight];
...
- (void)inputTextViewDidBeginEditing:(EaseTextView *)inputTextView {
    if (self.conversation.type == EMConversationTypeChat && self.isTyping) {
        ...
        EMMessage *msg           = [[EMMessage alloc] initWithConversationID:self.conversation.conversationId
                                                              from:from
...
- (BOOL)didInputAtInLocation:(NSUInteger)location {
    if ([self.delegate respondsToSelector:@selector(messageViewController:selectAtTarget:)] &&
        self.conversation.type == EMConversationTypeGroupChat) {
        location += 1;
        __weak typeof(self) weakSelf = self;
        [self.delegate
            messageViewController:self
                   selectAtTarget:^(EaseAtTarget *target) {
                       ...
                       if (strongSelf && target) {
                           if ([target.userId length] || [target.nickname length]) {
                               ...
                               NSString *insertStr = [NSString
                                   stringWithFormat:@"%@ ", target.nickname ? target.nickname
                                                                            : target.userId];
                               EaseChatToolbar *toolbar = (EaseChatToolbar *)strongSelf.chatToolbar;
                               NSMutableString *originStr =
                                   [toolbar.inputTextView.text mutableCopy];
                               NSUInteger insertLocation =
                                   location > originStr.length ? originStr.length : location;
                               [originStr insertString:insertStr atIndex:insertLocation];
                               toolbar.inputTextView.text = originStr;
                               toolbar.inputTextView.selectedRange =
                                   NSMakeRange(insertLocation + insertStr.length, 0);
    ...
    if ([toolbar.inputTextView.text length] == location + 1) {
        ...
        if (range.location != NSNotFound) {
            if (location - range.location > 1) {
                NSString *sub =
                    [inputText substringWithRange:NSMakeRange(range.location + 1,
                                                              location - range.location - 1)];
                for (EaseAtTarget *target in self.atTargets) {
                    if ([sub isEqualToString:target.userId] ||
                        [sub isEqualToString:target.nickname]) {
                        inputText =
                            range.location > 0 ? [inputText substringToIndex:range.location] : @"";
...
- (void)didSendText:(NSString *)text withExt:(NSDictionary *)ext {
    if ([ext objectForKey:EASEUI_EMOTION_DEFAULT_EXT]) {
        ...
        if (self.dataSource &&
            [self.dataSource
                respondsToSelector:@selector(emotionExtFormessageViewController:easeEmotion:)]) {
            NSDictionary *ext =
                [self.dataSource emotionExtFormessageViewController:self
                                                        easeEmotion:emotion];
            [self sendTextMessage:emotion.emotionTitle withExt:ext];
        } else {
            [self sendTextMessage:emotion.emotionTitle
                          withExt:@{
                              MESSAGE_ATTR_EXPRESSION_ID : emotion.emotionId,
                              MESSAGE_ATTR_IS_BIG_EXPRESSION : @(YES)
                          }];
        }
        return;
    }
    if (text && text.length > 0) { [self sendTextMessage:text withExt:ext]; }
}
        ...
        switch (recordResponse) {
            ...
            case EMCanRecord: {
                if ([weakSelf.delegate respondsToSelector:@selector
                                       (messageViewController:didSelectRecordView:withEvenType:)]) {
                ...
                [[EMCDDeviceManager sharedInstance]
                    asyncStartRecordingWithFileName:fileName
                                         completion:^(NSError *error) {
                                             if (error) {
                                                 NSLog(@"%@", NSEaseLocalizedString(
                                                                  @"message.startRecordFail",
            ...
            case EMCanNotRecord: {
                UIAlertView *alertView = [[UIAlertView alloc]
                        initWithTitle:NSEaseLocalizedString(@"prompt", @"Prompt")
                              message:NSEaseLocalizedString(@"record.failToPermission",
    ...
    if (_isRecording) {
        ...
        if ([self.delegate respondsToSelector:@selector
            ...
            [self.delegate messageViewController:self
                             didSelectRecordView:recordView
                                    withEvenType:EaseRecordViewTypeTouchUpOutside];
        } else {
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView {
    if (_isRecording) {
        if ([self.delegate respondsToSelector:@selector
            ...
            [self.delegate messageViewController:self
                             didSelectRecordView:recordView
                                    withEvenType:EaseRecordViewTypeTouchUpInside];
        } else {
            if ([self.recordView isKindOfClass:[EaseRecordView class]]) {
        ...
        __weak typeof(self) weakSelf = self;
        [[EMCDDeviceManager sharedInstance]
            asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration,
                                               NSError *error) {
                if (!error) {
...
- (void)didDragInsideAction:(UIView *)recordView {
    if ([self.delegate respondsToSelector:@selector
        ...
        [self.delegate messageViewController:self
                         didSelectRecordView:recordView
                                withEvenType:EaseRecordViewTypeDragInside];
    } else {
        if ([self.recordView isKindOfClass:[EaseRecordView class]]) {
...
- (void)didDragOutsideAction:(UIView *)recordView {
    if ([self.delegate respondsToSelector:@selector
        ...
        [self.delegate messageViewController:self
                         didSelectRecordView:recordView
                                withEvenType:EaseRecordViewTypeDragOutside];
    } else {
        if ([self.recordView isKindOfClass:[EaseRecordView class]]) {
...
- (void)moreView:(EaseChatBarMoreView *)moreView didItemInMoreViewAtIndex:(NSInteger)index {
    if ([self.delegate
            respondsToSelector:@selector(messageViewController:didSelectMoreView:AtIndex:)]) {
...
- (void)moreViewTakePicAction:(EaseChatBarMoreView *)moreView {
    ...
    [self showHint:NSEaseLocalizedString(@"message.simulatorNotSupportCamera",
                                         @"simulator does not support taking picture")];
...
- (void)didReceiveHasDeliveredAcks:(NSArray *)aMessages {
    for (EMMessage *message in aMessages) { [self _updateMessageStatus:message]; }
}
...
- (void)didReceiveHasReadAcks:(NSArray *)aMessages {
    for (EMMessage *message in aMessages) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector
                                    (messageViewController:didReceiveHasReadAckForModel:)]) {
    ...
    if (self.menuIndexPath && self.menuIndexPath.row > 0) {
        ...
        if (self.menuIndexPath.row - 1 >= 0) {
            ...
            if ((!nextMessage || [nextMessage isKindOfClass:[NSString class]]) &&
                ...
                [indexPaths addObject:[NSIndexPath indexPathForRow:(self.menuIndexPath.row - 1)
                                                         inSection:0]];
            }
        }
...
- (NSArray *)formatMessages:(NSArray *)messages {
    ...
    if ([messages count] == 0) { return formattedArray; }
    ...
    for (EMMessage *message in messages) {
        ...
        if (self.messageTimeIntervalTag < 0 || interval > 60 || interval < -60) {
            NSDate *messageDate = [NSDate
                dateWithTimeIntervalInMilliSecondSince1970:(NSTimeInterval)message.timestamp];
            NSString *timeStr = @"";

            if (_dataSource &&
                [_dataSource respondsToSelector:@selector(messageViewController:stringForDate:)]) {
        ...
        if (_dataSource &&
            [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            model = [_dataSource messageViewController:self modelForMessage:message];
        } else {
            model               = [[EaseMessageModel alloc] initWithMessage:message];
            model.avatarImage   = [UIImage ease_imageNamedFromMyBundle:@"user"];
            model.failImageName = @"imageDownloadFail";
        }

        if (model) { [formattedArray addObject:model]; }
    }
...
- (void)addMessageToDataSource:(EMMessage *)message progress:(id)progress {
    ...
    dispatch_async(_messageQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [weakSelf.tableView reloadData];
            [weakSelf.tableView
                scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[weakSelf.dataArray count] - 1
                                                          inSection:0]
...
- (void)_refreshAfterSentMessage:(EMMessage *)aMessage {
    if ([self.messsagesSource count] && [EMClient sharedClient].options.sortMessageByServerTime) {
        ...
        if ([last isKindOfClass:[EMMessage class]]) {
            ...
            index                    = NSNotFound;
            [self.messsagesSource
                enumerateObjectsWithOptions:NSEnumerationReverse
                                 usingBlock:^(EMMessage *obj, NSUInteger idx, BOOL *stop) {
                                     if ([obj isKindOfClass:[EMMessage class]] &&
                                         [obj.messageId isEqualToString:msgId]) {
                                         index = idx;
                                         *stop = YES;
                                     }
                                 }];
            if (index != NSNotFound) {
                ...
                [self.tableView reloadData];
                [self.tableView
                    scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.dataArray count] - 1
                                                              inSection:0]
                          atScrollPosition:UITableViewScrollPositionBottom
                                  animated:NO];
                return;
...
- (void)sendMessage:(EMMessage *)message isNeedUploadFile:(BOOL)isUploadFile {
    ...
    if (self.easeExt != nil && [self.easeExt.allKeys count] > 0) {
        if (message.ext == nil || [message.ext.allKeys count] == 0) {
            message.ext = self.easeExt;
        } else {
            NSMutableDictionary *messageExt =
                [[NSMutableDictionary alloc] initWithDictionary:message.ext];
            [messageExt mergingWithDictionary:self.easeExt];
        ...
        progress:^(int progress) {
            if (weakself.dataSource &&
                [weakself.dataSource
                    ...
                    (messageViewController:updateProgress:messageModel:messageBody:)]) {
                [weakself.dataSource messageViewController:weakself
    ...
    if (self.conversation.type == EMConversationTypeGroupChat) {
        ...
        if ([targets count]) {
            ...
            [targets enumerateObjectsUsingBlock:^(NSString *target, NSUInteger idx, BOOL *stop) {
                if ([target compare:kGroupMessageAtAll options:NSCaseInsensitiveSearch] ==
                    NSOrderedSame) {
                    atAll = YES;
                    *stop = YES;
                }
...
- (void)sendTextMessage:(NSString *)text withExt:(NSDictionary *)ext {
    EMMessage *message = [EaseSDKHelper getTextMessage:text
                                                    to:self.conversation.conversationId
...
- (void)sendLocationMessageLatitude:(double)latitude
                         ...
                         andAddress:(NSString *)address {
    EMMessage *message =
        [EaseSDKHelper getLocationMessageWithLatitude:latitude
                                            longitude:longitude
                                              address:address
                                                   to:self.conversation.conversationId
                                          messageType:[self _messageTypeFromConversationType]
...
- (void)sendImageMessageWithData:(NSData *)imageData {
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector
                                    (messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self
                   progressDelegateForMessageBodyType:EMMessageBodyTypeImage];
    } else {
        progress = self;
    }

    EMMessage *message =
        [EaseSDKHelper getImageMessageWithImageData:imageData
                                                 to:self.conversation.conversationId
                                        messageType:[self _messageTypeFromConversationType]
                                         messageExt:nil];
...
- (void)sendImageMessage:(UIImage *)image {
    ...
    EMMessage *message =
        [EaseSDKHelper getImageMessageWithImage:image
                                             to:self.conversation.conversationId
                                    messageType:[self _messageTypeFromConversationType]
                                     messageExt:nil];
    [self sendMessage:message isNeedUploadFile:YES];
...
- (void)sendVoiceMessageWithLocalPath:(NSString *)localPath duration:(NSInteger)duration {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector
                                    (messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self
                   progressDelegateForMessageBodyType:EMMessageBodyTypeVoice];
    } else {
        progress = self;
    }

    EMMessage *message =
        [EaseSDKHelper getVoiceMessageWithLocalPath:localPath
                                           duration:duration
                                                 to:self.conversation.conversationId
                                        messageType:[self _messageTypeFromConversationType]
                                         messageExt:nil];
...
- (void)sendVideoMessageWithURL:(NSURL *)url {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector
                                    (messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self
                   progressDelegateForMessageBodyType:EMMessageBodyTypeVideo];
    } else {
        progress = self;
    }

    EMMessage *message =
        [EaseSDKHelper getVideoMessageWithURL:url
                                           to:self.conversation.conversationId
                                  messageType:[self _messageTypeFromConversationType]
                                   messageExt:nil];
    [self sendMessage:message isNeedUploadFile:YES];
...
- (void)didBecomeActive {
    ...
    if (self.isViewDidAppear) {
        ...
        if (self.dataSource && [self.dataSource respondsToSelector:@selector
                                                (messageViewControllerMarkAllMessagesAsRead:)]) {
        ...
        for (int i = 0; i < self.dataArray.count; i++) {
            ...
            if ([object isKindOfClass:[EaseMessageModel class]]) {
                ...
                if ([message.messageId isEqualToString:model.messageId]) {
                    ...
                    if (self.dataSource &&
                        [self.dataSource
                            respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
                        model =
                            [self.dataSource messageViewController:self
                                                   modelForMessage:message];
                    } else {
                        model               = [[EaseMessageModel alloc] initWithMessage:message];
                        model.avatarImage   = [UIImage ease_imageNamedFromMyBundle:@"user"];
                        model.failImageName = @"imageDownloadFail";
                    ...
                    [self.tableView
                        reloadRowsAtIndexPaths:@[ [NSIndexPath indexPathForRow:i inSection:0] ]
...
- (void)_updateMessageStatus:(EMMessage *)aMessage {
    ...
    if (aMessage && isChatting) {
        ...
        if (model) {
            __block NSUInteger index = NSNotFound;
            [self.dataArray
                enumerateObjectsUsingBlock:^(EaseMessageModel *model, NSUInteger idx, BOOL *stop) {
            ...
            if (index != NSNotFound) {
                ...
                [self.tableView beginUpdates];
                [self.tableView
                    reloadRowsAtIndexPaths:@[ [NSIndexPath indexPathForRow:index inSection:0] ]
                          withRowAnimation:UITableViewRowAnimationNone];
                [self.tableView endUpdates];
            }
        }
        ...
        if ([splits count]) {
            for (NSString *split in splits) {
                if (split.length) {
                    ...
                    if (split.length >= atALl.length &&
                        [split compare:atALl
                               options:NSCaseInsensitiveSearch
                                 range:NSMakeRange(0, atALl.length)] == NSOrderedSame) {
                    ...
                    for (EaseAtTarget *target in self.atTargets) {
                        if ([target.userId length]) {
                            if ([split hasPrefix:target.userId] ||
                                (target.nickname && [split hasPrefix:target.nickname])) {
