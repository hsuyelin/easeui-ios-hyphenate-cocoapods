...
+ (void)initialize {
    ...
    cell.messageTextFont = [UIFont ease_mediumPingfangFontOfSize:15];
    cell.messageTextColor =
...
- (void)_setupSubviewsWithType:(EMMessageBodyType)messageType
    ...
    [_statusButton setImage:[UIImage ease_imageNamedFromMyBundle:@"messageSendFail"]
    ...
    _bubbleView =
            [[EaseBubbleView alloc] initWithMargin:isSender ? _rightBubbleMargin : _leftBubbleMargin
                                          isSender:isSender];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] &&
            [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case EMMessageBodyTypeImage: {
                ...
                _bubbleView.imageView.image =
                        [UIImage ease_imageNamedFromMyBundle:@"imageDownloadFail"];
            }
                break;
            case EMMessageBodyTypeLocation: {
                ...
                _bubbleView.locationImageView.image =
                        [[UIImage ease_imageNamedFromMyBundle:@"chat_location_preview"]
            ...
            case EMMessageBodyTypeVideo: {
                ...
                _bubbleView.videoTagView.image =
                        [UIImage ease_imageNamedFromMyBundle:@"messageVideo"];
            }
                break;
    ...
    UITapGestureRecognizer *tapRecognizer =
            [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bubbleViewTapAction:)];
    [_bubbleView addGestureRecognizer:tapRecognizer];

    UITapGestureRecognizer *tapRecognizer2 =
            [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(avatarViewTapAction:)];
...
- (void)_setupConstraints {
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.bubbleView
                                                     ...
                                                     attribute:NSLayoutAttributeBottom
                                                    multiplier:1.0
                                                      constant:-EaseMessageCellPadding]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead
                                                    ...
                                                    multiplier:1.0
                                                      constant:0]];
    //    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead
    //    attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self.activity
...
- (void)setModel:(id <IMessageModel>)model {
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] &&
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText: {
                _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance]
                        attStringFromTextForChatting:model.text
                                            textFont:self.messageTextFont];
            }
                break;
            case EMMessageBodyTypeImage: {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.imageView
                                sd_setImageWithURL:[NSURL URLWithString:_model.thumbnailFileURLPath]
                                  placeholderImage:[UIImage
                                          ease_imageNamedFromMyBundle:_model
                                                  .failImageName]];
                    } else {
            ...
            case EMMessageBodyTypeVoice: {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 &&
                            [self.recvMessageVoiceAnimationImages count] > 0) {
                        self.bubbleView.voiceImageView.image =
                                self.model.isSender
                                        ? [self.sendMessageVoiceAnimationImages objectAtIndex:0]
                                        : [self.recvMessageVoiceAnimationImages objectAtIndex:0];
                        _bubbleView.voiceImageView.animationImages =
                                self.model.isSender ? self.sendMessageVoiceAnimationImages
                                        : self.recvMessageVoiceAnimationImages;
                    } else {
                        self.bubbleView.voiceImageView.image =
                                self.model.isSender
                                        ? [UIImage
                                        ease_imageNamedFromMyBundle:@"chat_sender_audio_playing_full"]
                                        : [UIImage ease_imageNamedFromMyBundle:
                                        @"chat_receiver_audio_playing_full"];
                    }
                }
                ...
                _bubbleView.voiceDurationLabel.text =
                        [NSString stringWithFormat:@"%d''", (int) _model.mediaDuration];
            }
                break;
            case EMMessageBodyTypeVideo: {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.videoImageView
                                sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath]
                                  placeholderImage:[UIImage
                                          ease_imageNamedFromMyBundle:_model
                                                  .failImageName]];
                    } else {
                ...
                break;
            case EMMessageBodyTypeFile: {
                _bubbleView.fileIconView.image =
                        [UIImage ease_imageNamedFromMyBundle:_model.fileIconName];
...
- (void)setBubbleMargin:(UIEdgeInsets)bubbleMargin {
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] &&
            [self isCustomBubbleView:_model]) {
...
- (void)setMessageTextFont:(UIFont *)messageTextFont {
    ...
    if (_bubbleView.textLabel) {_bubbleView.textLabel.font = messageTextFont;}
}

- (void)setMessageTextColor:(UIColor *)messageTextColor {
    _messageTextColor = messageTextColor;
    if (_bubbleView.textLabel) {_bubbleView.textLabel.textColor = _messageTextColor;}
}

- (void)setMessageLocationColor:(UIColor *)messageLocationColor {
    _messageLocationColor = messageLocationColor;
    if (_bubbleView.locationLabel) {_bubbleView.locationLabel.textColor = _messageLocationColor;}
}

- (void)setMessageLocationFont:(UIFont *)messageLocationFont {
    _messageLocationFont = messageLocationFont;
    if (_bubbleView.locationLabel) {_bubbleView.locationLabel.font = _messageLocationFont;}
}
...
- (void)setMessageFileNameFont:(UIFont *)messageFileNameFont {
    ...
    if (_bubbleView.fileNameLabel) {_bubbleView.fileNameLabel.font = _messageFileNameFont;}
}

- (void)setMessageFileNameColor:(UIColor *)messageFileNameColor {
    _messageFileNameColor = messageFileNameColor;
    if (_bubbleView.fileNameLabel) {_bubbleView.fileNameLabel.textColor = _messageFileNameColor;}
}

- (void)setMessageFileSizeFont:(UIFont *)messageFileSizeFont {
    _messageFileSizeFont = messageFileSizeFont;
    if (_bubbleView.fileSizeLabel) {_bubbleView.fileSizeLabel.font = _messageFileSizeFont;}
}

- (void)setMessageFileSizeColor:(UIColor *)messageFileSizeColor {
    _messageFileSizeColor = messageFileSizeColor;
    if (_bubbleView.fileSizeLabel) {_bubbleView.fileSizeLabel.textColor = _messageFileSizeColor;}
}
...
+ (CGFloat)cellHeightWithModel:(id <IMessageModel>)model {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue == 7.0) {bubbleMaxWidth = 200;}
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            NSAttributedString *text = [[EaseEmotionEscape sharedInstance]
                    attStringFromTextForChatting:model.text
                                        textFont:cell.messageTextFont];
            ...
            //            CGSize retSize;
            //            if ([NSString
            //            instancesRespondToSelector:@selector(boundingRectWithSize:options:attributes:context:)])
            //            {
            //                retSize = [text boundingRectWithSize:CGSizeMake(bubbleMaxWidth,
            //                CGFLOAT_MAX)
            //                options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading
            //                attributes:@{NSFontAttributeName:textFont} context:nil].size;
            //            }else{
            //                retSize = [text sizeWithFont:textFont
            //                constrainedToSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX)
        ...
        case EMMessageBodyTypeFile: {
            ...
            (boundingRectWithSize:options:attributes:context:)]) {
                nameRect = [text boundingRectWithSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX)
                                              options:NSStringDrawingUsesLineFragmentOrigin
                                           attributes:@{NSFontAttributeName: font}
                                              context:nil];
            } else {
                nameRect.size = [text sizeWithFont:font
                                 constrainedToSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX)
                                     lineBreakMode:NSLineBreakByCharWrapping];
            }
            ...
            if ([NSString instancesRespondToSelector:@selector
            (boundingRectWithSize:options:attributes:context:)]) {
                sizeRect = [text boundingRectWithSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX)
                                              options:NSStringDrawingUsesLineFragmentOrigin
                                           attributes:@{NSFontAttributeName: font}
                                              context:nil];
            } else {
                sizeRect.size = [text sizeWithFont:font
                                 constrainedToSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX)
                                     lineBreakMode:NSLineBreakByCharWrapping];
            }
