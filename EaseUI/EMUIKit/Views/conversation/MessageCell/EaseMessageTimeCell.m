...
- (void)_setupTitleLabelConstraints {
    CGFloat timeWidth =
            [self.title length] == 0
                    ? 50.0
                    : [UILabel getWidthByHeight:30 title:self.title font:[UIFont systemFontOfSize:12]];

    // top
    NSLayoutConstraint *timeLabelTopConstraint =
            [NSLayoutConstraint constraintWithItem:self.titleLabel
                                         ...
                                         attribute:NSLayoutAttributeTop
                                        multiplier:1.0
                                          constant:EaseMessageTimeCellPadding];
    [self.marginConstraints addObject:timeLabelTopConstraint];

    // bottom
    NSLayoutConstraint *timeLabelBottomConstraint =
            [NSLayoutConstraint constraintWithItem:self.titleLabel
    ...
    // centerX
    NSLayoutConstraint *timeLabelCenterXConstraint =
            [NSLayoutConstraint constraintWithItem:self.titleLabel
    ...
    // width
    NSLayoutConstraint *timeLabelWidthConstraint =
            [NSLayoutConstraint constraintWithItem:self.titleLabel
...
- (void)_refreshTitleLabelConstraints {
    for (NSLayoutConstraint *constraint in self.titleLabel.constraints) {
        if (constraint.firstAttribute == NSLayoutAttributeWidth) {
            CGFloat timeWidth = [self.title length] == 0
                    ? 50.0
                    : [UILabel getWidthByHeight:30
                                          title:self.title
...
- (NSMutableArray *)marginConstraints {
    if (!_marginConstraints) {_marginConstraints = [NSMutableArray array];}
