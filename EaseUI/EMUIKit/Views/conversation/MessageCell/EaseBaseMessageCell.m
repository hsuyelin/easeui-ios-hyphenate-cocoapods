...
+ (void)initialize {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {cell.messageNameIsHidden = NO;}
...
- (void)layoutSubviews {
    ...
    _bubbleView.backgroundImageView.image =
            self.model.isSender ? self.sendBubbleBackgroundImage : self.recvBubbleBackgroundImage;
    switch (self.model.bodyType) {
        ...
        case EMMessageBodyTypeImage: {
            ...
            CGFloat margin = [EaseMessageCell appearance].leftBubbleMargin.left +
                    [EaseMessageCell appearance].leftBubbleMargin.right;
        ...
        case EMMessageBodyTypeVoice: {
            ...
            self.bubbleWithImageConstraint =
                    [NSLayoutConstraint constraintWithItem:self.bubbleView
                                                 ...
                                                 attribute:NSLayoutAttributeNotAnAttribute
                                                multiplier:1.0
                                                  constant:[EaseMessageCell appearance].voiceCellWidth];
            [self addConstraint:self.bubbleWithImageConstraint];
        }
            break;
        ...
        switch (self.model.messageStatus) {
            ...
            case EMMessageStatusSucceed: {
                ...
                if (self.model.isDing) {
                    _hasRead.hidden = NO;
                    _hasRead.text =
                            [NSString stringWithFormat:@"%@ 已读", @(self.model.dingReadCount)];
                    ...
                    _hasRead.hidden = NO;
                    _hasRead.text =
                            [NSString stringWithFormat:@"%@ %@", @(self.model.dingReadCount),
                                            NSEaseLocalizedString(@"hasRead", @"Read")];
                }
            }
                break;
...
+ (CGFloat)cellHeightWithModel:(id <IMessageModel>)model {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue == 7.0) {height = 15;}
