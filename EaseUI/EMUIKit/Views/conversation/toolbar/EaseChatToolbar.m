...
- (instancetype)initWithFrame:(CGRect)frame {
    self = [self initWithFrame:frame
            ...
            inputViewMaxHeight:150
                          type:EMChatToolbarTypeGroup];
    if (self) {}
...
- (void)_setupSubviews {
    ...
    _backgroundImageView.image = [[UIImage ease_imageNamedFromMyBundle:@"messageToolbarBg"]
                                  stretchableImageWithLeftCapWidth:0.5
                                  topCapHeight:10];
    ...
    // input textview
    _inputTextView = [[EaseTextView alloc]
    ...
    [styleChangeButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_record"]
                       forState:UIControlStateNormal];
    [styleChangeButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_keyboard"]
                       forState:UIControlStateSelected];
    [styleChangeButton addTarget:self
                          action:@selector(styleButtonAction:)
                forControlEvents:UIControlEventTouchUpInside];
    
    EaseChatToolbarItem *styleItem =
    [[EaseChatToolbarItem alloc] initWithButton:styleChangeButton withView:nil];
    ...
    [self.recordButton setBackgroundImage:[[UIImage ease_imageNamedFromMyBundle:@"chatBar_recordBg"]
                                           stretchableImageWithLeftCapWidth:10
                                           topCapHeight:10]
                                 forState:UIControlStateNormal];
    [self.recordButton
     setBackgroundImage:[[UIImage ease_imageNamedFromMyBundle:@"chatBar_recordSelectedBg"]
                         stretchableImageWithLeftCapWidth:10
                         topCapHeight:10]
    ...
    [self.recordButton addTarget:self
                          action:@selector(recordButtonTouchDragExit)
                forControlEvents:UIControlEventTouchDragExit];
    [self.recordButton addTarget:self
                          action:@selector(recordButtonTouchDragEnter)
    ...
    [self.recordButton addTarget:self
                ...
                forControlEvents:UIControlEventTouchUpInside];
    [self.recordButton addTarget:self
                          action:@selector(recordButtonTouchCancel)
    ...
    [self.faceButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_face"]
                     forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_faceSelected"]
                     forState:UIControlStateHighlighted];
    [self.faceButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_keyboard"]
                     forState:UIControlStateSelected];
    [self.faceButton addTarget:self
                        action:@selector(faceButtonAction:)
              forControlEvents:UIControlEventTouchUpInside];
    EaseChatToolbarItem *faceItem =
    [[EaseChatToolbarItem alloc] initWithButton:self.faceButton withView:self.faceView];
    
    // more
    self.moreButton = [[UIButton alloc] init];
    self.moreButton.accessibilityIdentifier = @"more";
    self.moreButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.moreButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_more"]
                     forState:UIControlStateNormal];
    [self.moreButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_moreSelected"]
                     forState:UIControlStateHighlighted];
    [self.moreButton setImage:[UIImage ease_imageNamedFromMyBundle:@"chatBar_keyboard"]
                     forState:UIControlStateSelected];
    [self.moreButton addTarget:self
                        action:@selector(moreButtonAction:)
              forControlEvents:UIControlEventTouchUpInside];
    EaseChatToolbarItem *moreItem =
    [[EaseChatToolbarItem alloc] initWithButton:self.moreButton withView:self.moreView];
...
- (UIView *)faceView {
    if (_faceView == nil) {
        _faceView =
        [[EaseFaceView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame),
                                                       self.frame.size.width, 180)];
        [(EaseFaceView *) _faceView setDelegate:self];
        _faceView.backgroundColor =
...
- (UIView *)moreView {
    if (_moreView == nil) {
        _moreView = [[EaseChatBarMoreView alloc]
                     initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width,
                                              80)
                     type:self.chatBarType];
        _moreView.backgroundColor =
        [UIColor colorWithRed:240 / 255.0 green:242 / 255.0 blue:247 / 255.0 alpha:1.0];
...
- (void)setRecordView:(UIView *)recordView {
    if (_recordView != recordView) {_recordView = recordView;}
}
    ...
    for (id item in inputViewLeftItems) {
        if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
            ...
            if (chatItem.button) {
                ...
                if (itemFrame.size.height == 0) {itemFrame.size.height = itemHeight;}
                
                if (itemFrame.size.width == 0) {itemFrame.size.width = itemFrame.size.height;}
...
- (void)_willShowInputTextViewToHeight:(CGFloat)toHeight {
    if (toHeight < self.inputViewMinHeight) {toHeight = self.inputViewMinHeight;}
    if (toHeight > self.inputViewMaxHeight) {toHeight = self.inputViewMaxHeight;}
    ...
    if (toHeight == _previousTextViewContentHeight) {
        ...
        if (_delegate &&
            [_delegate respondsToSelector:@selector(chatToolbarDidChangeFrameToHeight:)]) {
...
- (void)_willShowBottomView:(UIView *)bottomView {
    if (![self.activityButtomView isEqual:bottomView]) {
        ...
        if (self.activityButtomView) {[self.activityButtomView removeFromSuperview];}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    ...
    for (EaseChatToolbarItem *item in self.leftItems) {item.button.selected = NO;}
    
    for (EaseChatToolbarItem *item in self.rightItems) {item.button.selected = NO;}
    ...
    if ([text isEqualToString:@"\n"]) {
        ...
        if ([self.delegate respondsToSelector:@selector(didInputAtInLocation:)]) {
            if ([self.delegate didInputAtInLocation:range.location]) {
                [self
                 _willShowInputTextViewToHeight:[self _getTextViewContentH:self.inputTextView]];
                return NO;
            }
        }
        ...
        if (range.length == 1 &&
            [self.delegate respondsToSelector:@selector(didDeleteCharacterFromLocation:)]) {
...
- (void)selectedFacialView:(NSString *)str isDelete:(BOOL)isDelete {
    ...
    if (!isDelete && str.length > 0) {
        if (self.version >= 7.0) {
            ...
            [attr insertAttributedString:[[EaseEmotionEscape sharedInstance]
                                          attStringFromTextForInputView:str
                                          textFont:self.inputTextView.font]
        ...
        if (self.version >= 7.0) {
            if (chatText.length > 0) {
                ...
                if (chatText.length >= 2) {
                    ...
                    if ([EaseEmoji stringContainsEmoji:subStr]) {length = 2;}
                }
...
- (NSMutableAttributedString *)backspaceText:(NSMutableAttributedString *)attr
    ...
    NSRange range = [self.inputTextView selectedRange];
    if (range.location == 0) {return attr;}
                ...
                [_inputTextView.attributedText
                 ...
                 usingBlock:^(id value, NSRange range, BOOL *stop) {
                     if (value) {
                         ...
                         NSString *str =
                         [NSString stringWithFormat:@"%@", attachment.imageName];
...
- (void)sendFaceWithEmotion:(EaseEmotion *)emotion {
    if (emotion) {
        if ([self.delegate respondsToSelector:@selector(didSendText:withExt:)]) {
            [self.delegate didSendText:emotion.emotionTitle
                               withExt:@{EASEUI_EMOTION_DEFAULT_EXT: emotion}];
            [self _willShowInputTextViewToHeight:[self _getTextViewContentH:self.inputTextView]];;
        }
    }
}
...
- (void)styleButtonAction:(id)sender {
    ...
    if (button.selected) {
        ...
        for (EaseChatToolbarItem *item in self.leftItems) {
            if (item.button != button) {item.button.selected = NO;}
        }
