...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    if (newSuperview) {[self reloadEmotionData];}
}
...
- (EaseFacialView *)facialView {
    if (_facialView == nil) {
        _facialView =
                [[EaseFacialView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 150)];
...
- (void)_setupButtom {
    ...
    sendButton.frame =
            CGRectMake((kButtomNum - 1) * CGRectGetWidth(_facialView.frame) / kButtomNum,
                    CGRectGetMaxY(_facialView.frame), CGRectGetWidth(_facialView.frame) / kButtomNum,
...
- (void)_setupButtonScrollView {
    ...
    for (UIView *view in [_bottomScrollView subviews]) {[view removeFromSuperview];}
...
- (void)selectedFacialView:(NSString *)str {
    if (_delegate) {[_delegate selectedFacialView:str isDelete:NO];}
}

- (void)deleteSelected:(NSString *)str {
    if (_delegate) {[_delegate selectedFacialView:str isDelete:YES];}
}

- (void)sendFace {
    if (_delegate) {[_delegate sendFace];}
}

- (void)sendFace:(EaseEmotion *)emotion {
    if (_delegate) {[_delegate sendFaceWithEmotion:emotion];}
}

#pragma mark - public

- (BOOL)stringIsFace:(NSString *)string {
    if ([_facialView.faces containsObject:string]) {return YES;}
