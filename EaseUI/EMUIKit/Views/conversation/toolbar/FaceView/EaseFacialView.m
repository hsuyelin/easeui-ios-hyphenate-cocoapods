...
- (void)setImage:(UIImage *)image withTitle:(NSString *)title forState:(UIControlState)stateType {
    ...
    if ([NSString instancesRespondToSelector:@selector(sizeWithAttributes:)]) {
        titleSize =
                [title sizeWithAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:10]}];
    } else {
        titleSize = [title sizeWithFont:[UIFont systemFontOfSize:10]];
    }
    [self.imageView setContentMode:UIViewContentModeScaleAspectFit];
    [self setImageEdgeInsets:UIEdgeInsetsMake(0, 0.0, 20, 0)];
    ...
    [self setTitleEdgeInsets:UIEdgeInsetsMake(CGRectGetHeight(self.bounds) - 20, -image.size.width,
            0, 0.0)];
...
- (void)setEmotion:(EaseEmotion *)emotion {
    ...
    if ([emotion isKindOfClass:[EaseEmotion class]]) {
        if (emotion.emotionType == EMEmotionGif) {
            [_imageButton setImage:[UIImage ease_imageNamedFromMyBundle:emotion.emotionThumbnail]
                         withTitle:emotion.emotionTitle
        ...
        [_imageButton setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setImage:[UIImage ease_imageNamedFromMyBundle:@"faceDelete"]
                      forState:UIControlStateNormal];
        [_imageButton setImage:[UIImage ease_imageNamedFromMyBundle:@"faceDelete_select"]
...
@interface EaseFacialView () <UIScrollViewDelegate, UICollectionViewDelegate,
        UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,
        EaseCollectionViewCellDelegate> {
    CGFloat _itemWidth;
    CGFloat _itemHeight;
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.collectionView registerClass:[EaseCollectionViewCell class]
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    if (_emotionManagers == nil || [_emotionManagers count] == 0) {return 1;}
...
- (void)didSendEmotion:(EaseEmotion *)emotion {
    if (emotion) {
        if (emotion.emotionType == EMEmotionDefault) {
            if (_delegate) {[_delegate selectedFacialView:emotion.emotionId];}
        } else {
            if (_delegate) {[_delegate sendFace:emotion];}
        }
    } else {
...
- (void)loadFacialViewWithPage:(NSInteger)page {
    ...
    if (page == 0) {
        ...
        [_collectionView
                setContentOffset:CGPointMake(CGRectGetWidth(self.frame) *
                                ((int) (offSet.x / CGRectGetWidth(self.frame)) + 1),
                        0)
                        animated:NO];
    }
    //    [_collectionView setContentOffset:CGPointMake(CGRectGetWidth(self.frame)*2, 0)
    //    animated:NO];
}
